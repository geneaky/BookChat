...
import toy.bookchat.bookchat.domain.chatroom.api.v1.response.RoomGuest;
import toy.bookchat.bookchat.domain.chatroom.api.v1.response.RoomHost;
import toy.bookchat.bookchat.domain.chatroom.api.v1.response.RoomSubHost;
...
class ChatRoomControllerTest extends ControllerTestExtension {
  ...
  void 사용자의_채팅방_목록_조회_성공() throws Exception {
    ...
    List<UserChatRoomResponse> result = List.of(getChatRoomResponse(chatRoomEntity1, chatEntity1, bookEntity1),
        getChatRoomResponse(chatRoomEntity2, chatEntity2, bookEntity2),
        getChatRoomResponse(chatRoomEntity3, chatEntity3, bookEntity3));
    ...
    mockMvc.perform(get("/v1/api/users/chatrooms")
        ...
        .andDo(document("get-user-chatrooms",
            ...
            responseFields(
                ...
                fieldWithPath("userChatRoomResponseList[].hostProfileImageUrl").optional().type(STRING)
                    .description("방장 프로필 이미지"),
                fieldWithPath("userChatRoomResponseList[].hostDefaultProfileImageType").type(NUMBER)
                ...
                fieldWithPath("userChatRoomResponseList[].senderProfileImageUrl").optional().type(STRING)
                    .description("마지막 채팅 보낸 사람 프로필 이미지"),
                fieldWithPath("userChatRoomResponseList[].senderDefaultProfileImageType").type(NUMBER)
                    .description("마지막 채팅 보낸 사람 기본 프로필 이미지 타입"),
                ...
                fieldWithPath("userChatRoomResponseList[].lastChatDispatchTime").type(STRING)
                    .description("마지막 채팅 발송 시간")
            ).and(getCursorField())));
  }
  ...
  void 전체_채팅방_목록에서_검색_성공() throws Exception {
    ...
    when(chatRoomService.getChatRooms(any(), any())).thenReturn(chatRoomResponses);
