...
): PagingSource<Int, BookSearchResultDto>() {

    // load() : 데이터 로드 (사용자가 스크롤 할 때마다 데이터를 비동기적으로 가져온다.)
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, BookSearchResultDto> {
        ...
        val bookSearchResultDtos = response?.body()
...
// 데이터가 새로고침되거나 첫 로드 후 무효화되었을 때 키를 반환하여 load()로 전달
    override fun getRefreshKey(state: PagingState<Int, BookSearchResultDto>): Int? {
